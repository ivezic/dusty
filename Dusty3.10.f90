...
!=======================================================================
  ...
  if (maxrat.gt.accuracy) then 
     dejan_fac = 2.
     do iY = 2, nY
        Y1 = 0.5*(Y(iY)+Y(iY-1))
        tau1 = 0.5*(tauaux(iY)+tauaux(iY-1))
        Y2 = Y(nY)-Y1
        tau2 = tautot(1)-tau1
        dY = dabs(Y(iY)-Y(iY-1))
        dtau1 = (dejan_fac-dY/Y1)*tau1
        dtau2 = (dejan_fac-dY/Y2)*tau2
        deltaumax = dtau1
        if (ksi.gt.0) deltaumax = min(dtau1,dtau2)
        deltaumax = min(deltaumax,1.5d0*tautot(1)/nY)
        if (((tauaux(iY)-tauaux(iY-1)).gt.deltaumax).or.(dabs(1.-flux1(iY)/convF).gt.accuracy)) then
!           n_ins = int((tauaux(iY)-tauaux(iY-1))/deltaumax) 
           n_ins = 1
           if ((n_ins>3).or.(n_ins<1)) n_ins=3
           kins = kins + n_ins
           do i_ins = 1,n_ins
  ...
  ELSE
     ...
     if ((nY+kins).gt.npY) then
        ...
        do k = 1, kins
           ...
           do iG=1,1
              call shiftIns(Td(iG,:),npY,nY+k-1,Td(iG,iYins(k)+k-1),iYins(k)+k-1)
           end do
           do iL=1,nL
              call shiftIns(utot(iL,:),npY,nY+k-1,utot(iL,iYins(k)+k-1),iYins(k)+k-1)
           end do
        end do
     end if
  END IF
...
10 do while(fbolOK.eq.0)
     ...
     if(iVerb.eq.2.and.Lprint) write(*,'(A,F10.3,A)') '  Achieved error in bolometric Flux:',maxFerr*100,'%'
     IF(iInn.eq.1 .AND. Lprint) THEN
