!--**-- symbol used in case I use newer routine
!***********************************************************************
subroutine Kernel_matrix(nG,path,lpath,tauIn,tau,Nrec,Nmodel,GridType,error)
!=======================================================================
!=======================================================================
...
! ***********************************************************************
SUBROUTINE solve_matrix(model,nG,error)
! =======================================================================
...
!***********************************************************************
SUBROUTINE RADTRANSF(pstar,iPstar,TAUlim,nG,FbolOK,deviat,error,iterFbol,model)
!=======================================================================
! This subroutine solves the continuum radiative transfer problem for a
! spherically symmetric envelope.                      [Z.I., Nov. 1995]
!=======================================================================
  ...
  ELSE
     CALL Matrix(pstar,iPstar,mat0,mat1,mifront,miback)
  END IF
  Conv = 0
  iter = 0
...
!***********************************************************************
SUBROUTINE INVERT(nG,mat,Us,Em,omega,Uold,error)
!=======================================================================
...
!=======================================================================
  ...
  INTEGER iG,nG,iL,iY, iYaux, Kronecker, error
  DOUBLE PRECISION mat(npL,npY,npY), Us(npL,npY), Em(npG,npL,npY), &
       Uold(npL,npY),B(npY), A(npY,npY), X(npY),&
       omega(npG+1,npL)
  !--------------------------------------------------------------------
  error = 0
...
!***********************************************************************
SUBROUTINE matrix(pstar,iPstar,m0,m1,mifront,miback)
!=======================================================================
...
!=======================================================================
  use common
  IMPLICIT none
  INTEGER m,iP,nZ(npP), error, flag,iL,iY,iZ,jZ,im,iW,iPstar
  ...
  DO iL = 1, nL
     ...
     DO iP = 1, nP-1
        ...
        CALL WEIGHTS(TAUaux,iP,iL,nZ(iP),alpha,beta,gamma2,delta,wgmatp,wgmatm)
        ...
        DO iY = 1, nY
           ...
           DO iW = 1, nY
              ...
              IF (pstar.GT.0.0D+00) THEN
                 ...
                 IF (error.NE.0) GOTO 999
                 IF (nPcav.GT.iPstar) CALL NORDLUND(0,xN,yN,iPstar+1,nPcav+1,m,result1,error)
                 IF (error.NE.0) GOTO 999
              ...
              IF (iY.GT.1) THEN
                 CALL NORDLUND(flag,xN,yN,nPcav+1,Plast(iY),m,result2,error)
                 IF (error.NE.0) GOTO 999
              ELSE
                 result2 = 0.0D+00
              END IF
...
!***********************************************************************
SUBROUTINE WEIGHTS(TAUaux,iP,iL,nZ,alpha,beta,gamma2,delta,wgp,wgm)
!=======================================================================
