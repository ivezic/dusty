...
!! fixing the RDW output and minor editing in Solve, Sub Flux_Cons_Deka and Sub SPH_ext_illum.
!! 
...
!!
!!  5) Restored the old Ygrid and Pgrid routines, renamed Deka's to Ygrid_Deka, Pgrid_Deka. 
!!     Currently the grids are generated by subroutines Ygrid and Pgrid_Deka, and it works ok. 
!!
!!  6) Simplified subroutine Kernel (Deka's subroutine Core was renamed to Kernel_Deka).
!!
!!  7) Minor fixes: 
...
! Open master input file dusty.inp
...
! read input data
    call Input(nameIn,nG,nameOut,nameQ,nameNK,tau1,tau2,tauIn,Nrec,GridType,Nmodel,error,version,stdf)
...
!!     if (iVerb.eq.2) write(*,*) 'starting to solve'
!!**     call kernel_Deka(nG,path,lpath,tauIn,tau,Nrec,Nmodel,GridType,error,Lprint)
...
!***********************************************************************
subroutine Flux_Consv_Deka(flux1,flux2,fbolOK,error,Lprint)
!=======================================================================
! Replaces the former SUBROUTINE ChkFlux(flux,tolern,consfl,error,ETAzp) 
! 
! This subroutine checks the bolometric diffuse flux conservation at any 
...
!!**   end do
!!**
!!** all the above is simply the following:  [MN,'Aug'10]
     do iY = 1, nY
      tauaux(iY) = TAUtot(1)*ETAzp(1,iY)
     end do
...
777 return
end subroutine Flux_Consv_Deka
!*********************************************************************
...
!***********************************************************************
subroutine Init_Temp_Deka(nG,T_ext,us)
!=======================================================================
! This subroutine calculates the initial approximation for the temperature.
! Temperature is obtained by solving:
!   f2(y)*f1(y) - g(T) = 0
...
!=======================================================================
  ...
  integer iL, iY, iG, nG, iw
  double precision xP, Planck, us(npL,npY),T_ext(npY),fnum(npL),qp,ff(npL)
!--------------------------------------------------------------------------
...
! first approximation for temperature
    if(typentry(1).eq.5) then
! in this case Tsub(1) is given in input
! initialize the first element
     if(sph) then
      T_ext(iY) = Tsub(1)**4.0d0/Y(iY)**2.0d0
     else
      T_ext = Tsub(1)**4.0d0
     end if
     Td(iG,iY) = Tsub(1)
    else
! if flux fe1 is given in input in some form - at this point
! Tei is initialized already in input and carried in common/stellar/
     Td(iG,iY) = T_ext(iY)**(1.0d0/4.0d0)
    end if
   end do
...
! get intial temperature
     Td(iG,iY) = (f1*T_ext(iY)/(qp))**(1.0d0/4.0d0)
    end do
   end if
  end do
!-----------------------------------------------------------------------
...
return
end subroutine Init_Temp_Deka
!***********************************************************************


!***********************************************************************
subroutine Kernel_Deka(nG,path,lpath,tauIn,tau,Nrec,Nmodel,GridType,error,Lprint)
!=======================================================================
! This subroutine generates solution for all optical depth steps starting 
! with intial optical depth given in the inout file. It prints output 
...
!=======================================================================
  ...
  integer nG, i,j,iL, iG, GridType,model,error,Nmodel,Nrec, &
       iterfbol,fbolOK,diff,istop,lpath,imodel
  character*235 path
  double precision faux1(npL), faux2(npL), sigAfid, sigSfid, q,  &
       tauIn(Nrec),tau(Nmodel),tau1,ratio,delta, &
       tau0
...
! intialize print and initial flag
   Lprint = .false.
   initial = .false.
!  j is the counter over initial (smallest) optical depth [MN]
   j = j + 1   
   if (j.eq.1) then
    initial = .true.
    if (i.eq.1) then
...
! Extinction efficiency at fiducial wavelength
   ...
   do iL = 1, nL
    tautot(iL) = taufid0*(sigmaA(1,iL) + sigmaS(1,iL))/sigexfid
...
!!   if(tau0.eq.tau(model)) call OPPEN(model,path,lpath)
   if(Lprint) call OPPEN(model,path,lpath)
...
! if flux is not conserved, close all opened output files   
   if (fbolOK.ne.1) then
    close(15)
    close(16)
    close(17)
...
! if solution is not obtained, reduce the optical depth step.
   tau0 = tau0*dtau**(1.0d0/dfloat(i))

! end loop over optical depths
  end do
!-----------------------------------------------------------------------

10  return
end subroutine Kernel_Deka
!***********************************************************************
...
!!**   call Flux_Consv(calc_fdiff,comp_fdiff_bol,fbolOK,error,Lprint)
   call Flux_Consv_Deka(calc_fdiff,comp_fdiff_bol,fbolOK,error,Lprint)
...
!***********************************************************************
subroutine SPH_diff_Deka(flag1,initial,iter,iterfbol,T_ext,vec1,omega,vec2)
!=======================================================================
! This subroutine finds the diffuse part for both emission and scattering
! for spherical shell                                        [Deka, 2008]
!! If flag1=1 for vec1=Em; flag1=2 for vec1=Us  
...
!=======================================================================
  ...
  implicit none
  integer i,j,iP,iL,iZ,iZz,iY,iYy,n,nn,nZ,iter,iterfbol,iNloc,flag1, iW, flag, &
          error
  double precision vec1(npL,npY),vec2(npL,npY),tauaux(npY), angle(npP), &
       diff(npY),S_fun(npY), result1,result2,T_ext(npY),omega(npG,npL),dyn2,eta, &
       aux1(npP),aux2(npY),Yloc1, p_loc,expow1, expow2, x1,x2,p1,z(nP,nY), &
       Iplus1(npP,npL),Iplus2(npP,npL), Iminus(npP,npL), faux1, faux2, S_loc, &
	   func(npY), faux3(npY), res1, xN(npP), yN(npP)
...
!!** for each radial grid point calculate the integrals from Sec.4.1 in Blueprint:  
  do iY = 1, nY
   do iP = 1, Plast(iY)
     iZz  = iY + 1 - iYfirst(iP)   !this is z in eq.(4.1.5)
!    upper limit for the counter of z position
     nZ  = nY + 1 - iYfirst(iP)   !nZ is index for zmax=sqrt(Y**2-p**2) [MN]
     call find_z(iP,nZ,z)
!    intialize optical depth along the ray path for each P.  
     tauaux = 0.0d0
!    Now find optical depth along the ray path for each P.
     p1 = P(iP)
     do iZ = 1, nZ-1
       x1 = z(iP,iz)
       x2 = z(iP,iz+1)
       ...
       result1 = 0.0d0
       do i = 1, nn
         Yloc1 = dsqrt(xg(i)**2.0d0 + p1**2.0d0)
         result1 = result1 + eta(Yloc1)*wg(i)
       end do
       tauaux(iZ+1) = tauaux(iZ) + abs(result1)   !this is IntEta in old Dusty [MN].
     end do
...
!      in the shell 
       do iZ = 1, nZ-1
         ...
         if(iZ.eq.1.and.P(iP).gt.Y(iYy).and.P(iP).lt.Y(iYy+1)) then
            p_loc = P(iP)
!!**        changing this linear interpolation with power low or spline didn't matter here[MN]
            call LININTER(npY,nY,Y,S_fun,p_loc,iNloc,S_loc)
            diff(iZ) = abs(S_loc)
         else
            diff(iZ) = S_fun(iYy)
     ...
     do iZ = 1, nZ
      aux2(iZ) = tautot(iL)*abs(tauaux(iZ)) !this is tauTot*IntEta 
     end do
...
! from z0-midpoint to the running z-point 
     DO iZ = 1, iZz
	    expow1 = tautot(iL)*abs(tauaux(iZz) - tauaux(iZ))  !this is tau(z,z';p)
	    faux3(iZ) = exp(-expow1)
        func(iZ) =  diff(iZ)
     END DO
     CALL Simpson(npY,1,iZz,faux3,func,res1)                                                       
...
! from the running z-point to the outer edge of the shell [MN]
     DO iZ = iZz, nZ
	    expow1 = tautot(iL)*abs(tauaux(iZ) - tauaux(iZz)) !this is tau(z',z;p)
	    faux3(iZ) = exp(-expow1)
        func(iZ) =  diff(iZ)
     END DO
     CALL Simpson(npY,iZz,nZ,faux3,func,res1)                                                       
...
! Find diffuse energy density   
   if (moment_loc.eq.1) then
       do iL = 1, nL 
         do iP = 1, Plast(iY)
           angle(iP) = asin(P(iP)/Y(iY))
           aux1(iP) =  (P(iP)/Y(iY))*abs(Iplus1(iP,iL) + Iplus2(iP,iL) + Iminus(iP,iL))
         end do  
         call Simpson(Plast(iY),1,Plast(iY),angle,aux1,result1)
         vec2(iL,iY) = 0.5*abs(result1)/T_ext(iY)
       end do
! ------------end of U integration ------------------
! Find diffuse flux
   elseif(moment_loc.eq.2) then
       do iL = 1, nL
         do iP = 1, Plast(iY)
          angle(iP) = asin(P(iP)/Y(iY))
          aux1(iP) = (Iplus1(iP,iL) + Iplus2(iP,iL))! - Iminus(iP,iL))
         end do      
         result1 = 0.0d0
         do iP = 1, Plast(iY)-1
           x1 = angle(iP)
           ...
           do i = 1, nn
            p_loc = xg(i)
            call LinInter(Plast(iY),Plast(iY),angle,aux1,p_loc,iNloc,Yloc1)
            result1 = result1 + Yloc1*wg(i)*sin(xg(i))*cos(xg(i))
           end do
         end do

         do iP = 1, Plast(iY)
          aux1(iP) = (Iminus(iP,iL))
         end do
...
!        call Simpson(Plast(iY),1,Plast(iY),angle,aux1,result1) !Deka commented these
         vec2(iL,iY) = 2.0d0*pi*abs(result1-result2)/T_ext(iY)!/Y(iY)**2.0d0
       end do !end do over lambda(iL)
   end if !end if for diffuse flux

  end do !end do over radial grid(iY)  [MN]
!-----------------------------------------------------------------------

999  return
end subroutine SPH_diff_Deka
!***********************************************************************
...
!    call find_z(iP,nZ,z)
!!   intialise optical depth along the ray path for each P.
!    tauaux = 0.0d0
!!   Now find optical depth along the ray path for each P.
!    p1 = P(iP)
!    do iz = 2, nZ
...
!      result1 = result1 + eta(Yloc1)*wg(i)
!     end do
!     tauaux(iz) = tauaux(iz-1) + abs(result1)
!    end do
!!** Equivalently [MN]:
    do iZ = 1, nZ
...
!!**    call Ygrid_Deka(pstar,iPstar,error)
! generate p and z grid
!!**    call Pgrid(pstar,iPstar,error)
    call Pgrid_Deka(pstar,iPstar,error)
   else 
...
!=======================================================================
! This is the old p-grid routine developed by [Zeljko Ivezic'95]    . 
!                                                              [MN'10]
!=======================================================================
...
!     insert two rays if the stellar disk size is finite                                            
      IF (pstar.GT.0.0) THEN                                                                        
        ...
        iPstar = 0                                                                                  
      ENDIF                                                                                         
!     impact parameters for the envelope (tangential to the radial grid)                            
      DO i =1, nY-1                                                                                 
        Plast(i) = iP + 1                                                                           
!       points close to the inner cavity should have more impact param.                             
        NinsLoc = Nins                                                                              
!       if tau is large add a few more points to the impact param. grid                             
...
subroutine Ygrid_Deka(pstar,iPstar,error)
!=======================================================================
! This subroutine generates the radial (Y) grid. Yout is the relative 
! thickness, Yout=rout/r1. pstar is the impact parameter for star (0<=p<=1). 
! First few points are prescribed arbitrarily.  This subroutine calls 
...
!=======================================================================
  ...
  use common
  implicit none  
  integer i,j, jy,itr,istop,error,iPstar, iter, iYdummy, iY
  double precision pstar, dyr, dyt, eta, aux, etatemp(npY), &
       ee, Yloc,  yold, Ynew, y1, delY1(npY), ymid,      &
       rat, dy, etamin, etamax,deltau, delY,tauloc2
  external eta
!-----------------------------------------------------------------------
...
! save old grid and values of eta (important for denstyp = 5 or 6)
  ...
  y1 = 1.0d0
  Y(1) = y1
! define the second grid point
!!** [MN] pow is the power of density power low, entered in .inp file
  Y(2) = Y(1) + 0.001d0**(pow/2.0d0)
  i = 2
  delY = (Y(i)-Y(i-1))
...
! for broken power-laws
  ...
  istop = 0
  ymid = (Y(i)+Y(i-1))/2.0d0
!  ymid = (Y(i)*Y(i-1))**(1.0d0/2.0d0)
  tauloc2 = taumax*eta(ymid)*(Y(i)-Y(i-1))
  do while(istop.ne.1)
   ymid = (Y(i)+Y(i-1))/2.0d0
!   ymid = sqrt(Y(i)*Y(i-1))
   deltau = taumax*eta(ymid)*(Y(i)-Y(i-1))
   tauloc2 = tauloc2 + deltau
   i = i + 1
   delY = 2.0d0**(pow - 1.0d0)*tauloc2*Y(i-1)/(taumax*eta(ymid)*Y(i-1) + tauloc2)
   Y(i) = Y(i-1) + delY
! make sure that all transition points are included in the grid
   if (ntr.ge.1) then
    if (Y(i).ge.Ytr(itr)) then
   ...
   Ynew = Yout/3.0d0 !(pow + 1.0d0) !(2.0d0)**(pow)
   if(Y(i).ge.Ynew) then
    istop = 1
    Y(i) = Ynew
    nY = i
...
! check that the y grid is not too large
  if (nY.gt.npY) then
   ...
   if (iX.ne.0) then
    call line(0,2,18)
    write(18,'(a)') ' ****************   WARNING   *******************'
    write(18,'(a46,i3)') ' nY is greater than npY:',nY
    write(18,'(a,i3,a)')' You need to increase npY in userpar.inc'
    write(18,'(a)') ' ****************  giving up  *******************'
   end if
  end if
...
102 return
end subroutine Ygrid_Deka
!***********************************************************************

!***********************************************************************

subroutine Pgrid_Deka(pstar,iPstar,error)
!=======================================================================
...
!***********************************************************************
subroutine sph_int_Deka(nG,omega,fs)
!=======================================================================
! This is the former  SUBROUTINE FindInt(nG,ETAzp), modified by M.Deka  [MN].                                                                 
! This subroutine finds the intensity distribution at outer edge and for
! user specified wavelengths lamOut. It also evaluates the angular size
! of the stellar disk and adds two impact parameters describing the star
! to the P grid, thus producing bOut grid. All intensities are indeed
...
! =======================================================================
  ...
  integer iL, nn, izz,nG,iY,k,i,iLout,iLstop, iP, iw, nZ, nZpt, iZ, iZloc
  double precision eta,qaux(npL), tauaux(npP,npY),psi, alpha(1,npY),  &
!!!  double precision eta,qaux(npL), psi, alpha(1,npY),  &
       resaux, qutot1,qpTsub, xp, pst, stelfact, Istell(npL), Planck, &
       w1_loc, Intl, intr, xx , alb, Ids(npL,npP), Ide(npL,npP),w2,   &
       lw12, numcorr, etazpstar, qaux2(npL), z1, z2,   &
       delz, zloc, wloc, resint, pt, tz, idboth, tzp(100), pUtot,     &
       Semis(100), Sscat(100), inteta, palb, palf, alfa, exterm,      &
       UtotLoc, Sstem(100), Sstsc(100), Istem(npL,100), Idfront,      &
       Istsc(npL,100), deltau, factaux, UtotL, UtotR, ep1, tauzp1,    &
       tauInf, fnum(npL), fdenum(npL), res, denum, fs(npL,npY),       &
       omega(1,npL), x1, x2, p1, result1, z(npP,npY), Yloc1
  double precision, dimension(:), allocatable:: xg,  wg
  external eta
! -----------------------------------------------------------------------
...
! generate diffuse intensities, Ide (emission) and Ids (scattering)
! loop over impact parameters
  do iP = 1, Plast(nY)
    iZz  = iY + 1 - iYfirst(iP)
!   maximal number of points along tangential position, z
    ...
    do iZ = 2, nZ
      z1 = z(iP,iZ-1)
      z2 = z(iP,iZ)
      nn = 2*int(abs(z2 - z1)/10.0d0) + 11
      if(allocated(xg)) deallocate(xg)
      if(allocated(wg)) deallocate(wg)
      allocate(xg(nn))
      allocate(wg(nn))
      call gauleg(z1,z2,xg,wg,nn)
      result1 = 0.0d0
      do i = 1, nn
       Yloc1 = dsqrt(xg(i)**2.0d0 + p1**2.0d0)
       result1 = result1 + eta(Yloc1)*wg(i)
      end do
      tauaux(iP,iZ) = tauaux(iP,iZ-1) + abs(result1)
!!**   print*, iP, iZ, tauaux(iP,iZ)
    end do
...
! index for the ending local radius
     iw = iYfirst(iP) + iZ - 1
! corresponding displacements along a line of sight
     z1 = z(iP,iZ-1)  !dsqrt(abs(w1_loc**2.0d+00-P(iP)**2.0d+00))
     z2 = z(iP,iZ)    !dsqrt(abs(w2**2.0d+00-P(iP)**2.0d+00))
! # of pts. for z integration, should increase with deltatau
! it is messy because int function which would do the job is
! not in f77 standard set
     nZpt = 5
...
! integrate between adjacent grid points
     ...
     do iZloc = 2, nZpt
      ...
      allocate(xg(nn))
      allocate(wg(nn))
      call gauleg(z1,zloc,xg,wg,nn)
      result1 = 0.0d0
      do i = 1, nn
       Yloc1 = dsqrt(xg(i)**2.0d0 + p1**2.0d0)
       result1 = result1 + eta(Yloc1)*wg(i)
      end do
      tzp(iZloc) = tzp(iZloc-1) + abs(result1)
     end do

! find local tauzp(w(z))-tauzp(w1_loc=w(z1))
     tzp(iZloc) = tzp(iZloc)*tautot(iL) - tauzp1
! find tz = t(zloc) = t(wloc), this works for 
! single size grains only; for multigrain case one needs
! to get Semis by summation over all Td
     tz = Td(1,iw-1) * (Y(iw-1)/wloc)**pt
...
! source functions (wloc**2 because d uses scaled quant.)
     ...
     Semis(iZloc) = (1.0d+00-omega(1,iL))* alfa *Planck(xP)/(4.0d+00 * pi)
     Sscat(iZloc) = omega(1,iL)*UtotLoc/(4.0d+00 * pi)
! check for the dynamic range
     call ChkRange(dynrange,Semis(iZloc))
     call ChkRange(dynrange,Sscat(iZloc))
! optical depth from infinity along the line of sight
...
!!**  write(*,*) ' Plast(nY) iP tauZout(iP), intens(17,iP)'
!!**  DO iP = 1, Plast(nY)
!!**    write(*,'(2i4,1p,2e12.3))') Plast(nY), iP, tauZout(iP), intens(17,iP)
!!**  END DO
!!**  stop
! -----------------------------------------------------------------------
999 return
end subroutine sph_int_Deka
! ***********************************************************************
